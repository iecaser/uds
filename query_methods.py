...
class DualDensityBase(QueryMethod):
    ...
    def get_embedding(self, X_labeled, X_unlabeled, batch_size=128):
        embedding_model = Model(inputs=self.model.input,
                                outputs=self.model.get_layer('coding').output)
        L = embedding_model.predict(X_labeled, batch_size=batch_size)
        U = embedding_model.predict(X_unlabeled, batch_size=batch_size)
    ...
    def query(self, X_train, Y_train, labeled_idx, amount):
        # params
        MIN_DISTANCE, MIN_SIMILARITY, MIN_SCORE = 0, 0, -9999
        ...
        for i in tqdm(range(amount)):
            ...
            score[selected_indices] = MIN_SCORE
...
class UncertaintyDensity(DualDensityBase):
    ...
    def cal_score(self, distance_to_labeled, similarity_to_unlabeled,
        ...
        score = np.log(similarity_to_unlabeled + 1e-10)\
...
class DynamicUncertaintyDualDensity(DualDensityBase):
    """By zxf"""
    ...
    def cal_score(self, distance_to_labeled, similarity_to_unlabeled,
        ...
        assert distance_to_labeled.shape\
            == similarity_to_unlabeled.shape\
            == uncertainty_of_unlabeled.shape
        coef = min(0.1 / uncertainty_of_unlabeled.mean(), 10)
        ...
        while np.sum(true_predictions != adversarial_predictions) < amount:
            ...
            for i in range(0, unlabeled.shape[0], 100):
                ...
                if i+100 > unlabeled.shape[0]:
                    ...
                    adversarial_images[i:i +
                                       100] = deep_fool.generate_np(unlabeled[i:i+100], **deep_fool_params)
    ...
    def query(self, X_train, Y_train, labeled_idx, amount):
        ...
        while labeled_so_far < amount:
            ...
            model = train_discriminative_model(
                X_train[labeled_idx], X_train[unlabeled_idx], self.input_shape, gpu=self.gpu)
...
class DiscriminativeRepresentationSampling(QueryMethod):
    ...
    def query(self, X_train, Y_train, labeled_idx, amount):
        ...
        representation = embedding_model.predict(
            X_train, batch_size=128).reshape((X_train.shape[0], -1, 1))
        ...
        while labeled_so_far < amount:
            ...
            model = train_discriminative_model(
                representation[labeled_idx], representation[unlabeled_idx], representation[0].shape, gpu=self.gpu)
...
class DiscriminativeStochasticSampling(QueryMethod):
    ...
    def query(self, X_train, Y_train, labeled_idx, amount):
        ...
        embedding_model = Model(inputs=self.model.input,
                                outputs=self.model.get_layer('coding').output)
        representation = embedding_model.predict(
            X_train, batch_size=256).reshape((X_train.shape[0], -1, 1))
...
class CoreSetMIPSampling(QueryMethod):
    ...
    def query_regular(self, X_train, Y_train, labeled_idx, amount):
        ...
        model, graph = self.mip_model(representation, labeled_idx, len(
            labeled_idx) + amount, upper_bound, outlier_count, greedy_indices=new_indices)
        ...
        while upper_bound - lower_bound > eps:
            ...
            print("upper bound is {ub}, lower bound is {lb}".format(
            ...
            if model.getAttr(gurobi.GRB.Attr.Status) in [gurobi.GRB.INFEASIBLE, gurobi.GRB.TIME_LIMIT]:
                ...
                gc.collect()
                model, graph = self.mip_model(representation, labeled_idx, len(
                    labeled_idx) + amount, current_delta, outlier_count, greedy_indices=indices)
                points, outliers = model.__data
                model.Params.SubMIPNodes = submipnodes
...
class EGLSampling(QueryMethod):
    ...
    def compute_egls(self, unlabeled, n_classes):
        ...
        self.get_gradient_length = K.function(
            [K.learning_phase(), self.input_placeholder, self.output_placeholder], [gradient_length])
